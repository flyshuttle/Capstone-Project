...
var pi = Math.PI;
  ...
  function arms_not_on_hips(kps){
    ...
    if((angle1>=50 && angle1<=90) || (angle2>= 50 && angle2 <=90)){
      return false
    }
    return true
  }
...
var ins_speak = {
  'Please make sure you are completely visible in the frame': false,
  'Asana overview': false,
  'Bring feet together' : false,
  'Rest arms on hip' : false,
  'Spread feet apart' : false,
  'Twist hip' : false,
  'Raise your arms' : false,
  'Lower into a lunge' : false,
  'keep arms raised' : false
}

function speak(ins){
  if(!ins_speak[ins]){
    responsiveVoice.speak(ins);
    ins_speak[ins] = true;
  }
}  
...
function detectPoseInRealTime(video, net) {
  ...
  async function poseDetectionFrame() {
    ...
    if(!checks_met['all_kps'] && not_all_kps_are_visible(kps) && current_stage<6){ 
      draw_text(ctx, 'Please make sure you are completely visible');
      speak('Please make sure you are completely visible in the frame');
    }
    else if(current_stage==0){
      checks_met['all_kps'] = true
      draw_image(ctx, pose_overview, 500, 0);
      draw_text(ctx, 'Asana overview');
      speak('Asana overview');
      setTimeout(function(){ current_stage=1; }, 10000);
    }
    else if(current_stage==1){
      draw_image(ctx, stage_1, 850, 0)
      if(!checks_met['stg_1_1'] && feet_not_together(kps)){
        draw_text(ctx, 'Bring feet together');
        speak('Bring feet together');
      }
      else if(!checks_met['stg_1_2'] && arms_not_on_hips(kps)){
        checks_met['stg_1_1'] = true
        draw_text(ctx, 'Rest arms on hip')
        speak('Rest arms on hip');
      }
      else{
        checks_met['stg_1_2'] = true
        current_stage = 2;
      }
    }
    else if(current_stage==2){
      draw_image(ctx, stage_2, 850, 0)
      if(!checks_met['stg_2_1'] && arms_not_on_hips(kps)){
        draw_text(ctx, 'Rest arms on hip')
        speak('Rest arms on hip');
      }
      else if(!checks_met['stg_2_2'] && feet_not_spread_apart(kps)){
        checks_met['stg_2_1'] = true
        draw_text(ctx, 'Spread feet apart')
        speak('Spread feet apart');
      }
      else{
        checks_met['stg_2_2'] = true
        current_stage = 3;
      }
    }
      ...
      if(!checks_met['stg_5_1'] && not_in_lunge_position(kps)){
        draw_text(ctx, 'Lower into a lunge')
        speak('Lower into a lunge');
      }
      else if(!checks_met['stg_5_2'] && arms_not_raised(kps)){
        checks_met['stg_5_1'] = true
        draw_text(ctx, 'keep arms raised')
        speak('keep arms raised');
      }
      else{
        checks_met['stg_5_2'] = true
        current_stage = 6;
      }
    }
